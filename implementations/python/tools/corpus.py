...
SETTLED = {
    ...
    "local-extra-field-unread": {
        "asked": {"verdict": "VERIFIED"},
        "note": "Both implementations agree, and the agreement is a finding this corpus did not have before: the file verifies. An extra field is in section 3.4's list of the things every byte of the file is accounted for by, nothing reads what is inside it, and both readers accept it — so a four-byte field a writer can fill with anything is a region of a charter/0.1 artifact whose bytes can be rewritten without a single thing the verdict reports changing. Section 3.6's last paragraph is the sentence that argues with that rule, and the case is recorded rather than settled because settling it changes the set of files that verify, which section 14's list makes a decision about the format rather than a patch: either an extra field is bytes the layout accounts for and nothing compares (what this artifact says), or a charter may not carry one and this case's answer becomes a defect. The case is here so that the day the format decides, the decision has an artifact to move, and its expected answer is the one the two implementations give today.",
    },
}
