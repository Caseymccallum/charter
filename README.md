...
That installs the `charter` command. It reads the repository directly, so it needs
no registry account and is the recommended install today.

- **From a clone, no install at all.** Every command in this file also works as
  `node cli/charter.js …`, which is the form the rest of this documentation uses.
  Nothing here needs the package to be linked or installed.
- **The package name is not the command name.** The command is `charter`; the
  package is `charter-cli`, and the `bin` field is what connects them. `charter` on
  npm is held by an unrelated, abandoned package, so the name is not available —
  [NAMING.md](NAMING.md) records that check and the alternatives with the reasons
  each was passed over.
- **Not published to the registry yet.** When it is, `npm install -g charter-cli`
  will be the shorter form of the same install. Until then that command does not
  resolve, which is why it is not the one printed above.
...
node cli/charter.js keygen -o key.key --encrypt              # the same key, behind a passphrase
node cli/charter.js seal notes.md --key key.key --passphrase-file pw.txt -o notes.charter
...
node cli/charter.js open notes.charter -o notes-copy.md      # the document back out, byte for byte
...
installed — see [Install the command](#install-the-command) — but nothing here needs
it to be.
...
A `.charter` file carries only the public key, so the private key lives on a disk,
where anyone who can read that file can sign as its holder. `keygen` writes it in
either of two forms, and the bytes say which:

| Written by | The file | Readable by |
| --- | --- | --- |
| `keygen -o key.pem` | an unencrypted PKCS#8 PEM | anyone who can read it, and by `openssl` |
| `keygen -o key.key --encrypt` | a passphrase-protected key file (SPEC.md §15.7) | whoever holds the passphrase |

The protected form is scrypt at `N = 2^17`, `r = 8`, `p = 1` — **128 MiB and about a
third of a second per attempt** — with AES-256-GCM over the key, so a file that has
been altered is refused rather than decrypted into something else. Both of those
are deliberate, and the reason is the one thing worth understanding about a
passphrase-wrapped key:

> **An attacker who has the file can guess at it forever, offline, and nothing in
> the file can stop them.** The only lever is what each guess costs, and the only
> thing that makes a guess expensive is the passphrase's length. Everything else here
> is arithmetic.

The passphrase comes from `--passphrase-file <file>` (its first line),
`CHARTER_PASSPHRASE` in the environment, or a prompt on a terminal. **A passphrase
passed on the command line is refused**, for every verb, because a shell keeps its
arguments in its history and in the process list, and both outlive the command.
The tool says so rather than quietly accepting one.

The editor cannot read a protected key: a browser's Web Crypto takes no passphrase
for a PKCS#8 import and the page implements no key derivation of its own, so a
protected key is refused there with that reason. Seal and edit from the command
line, or paste an unencrypted key into the page and accept that it is unencrypted.
...
npm test                # the 220 tests, through node --test
...
The command line is one caller. If you are writing the other kind — an editor plugin, a
build step, a pre-commit hook, an ingest queue, a page — the same checks are importable:

```js
import { verify, VERDICT, REASON, CAVEATS } from 'charter-cli';

const result = await verify(bytes);            // bytes, not a path: this reads no files
if (result.verdict !== VERDICT.VERIFIED) {
  for (const check of result.checks.filter((c) => c.status === 'FAIL')) {
    console.log(check.id, check.reason_code);   // the vocabulary, not the prose
  }
}
```

Two entry points, and the split is the project's own boundary rather than a packaging
convenience:

| Entry point | Names |
| --- | --- |
| `charter-cli` | `verify`, `ENTRY_MANIFEST`, `ENTRY_CONTENT`, `ENTRY_PROVENANCE`, `REQUIRED_ENTRIES`, `FORMAT`, `ALGORITHM`, `STATUS`, `VERDICT`, `EXIT_CODE`, `LEVEL`, `REASON`, `CHECK_REGISTRY`, `CHECK_IDS`, `CAVEATS`, `LIMITS`, `RefusalError`, `isReasonCode` |
| `charter-cli/producer` | `seal`, `edit`, `loadKey`, `generateKeyPair`, `openKeyFile`, `sealKeyFile`, `isKeyFile`, `readCharter`, `readCharterParts`, `citationItem`, `ProducerError`, `LATER_ACTION`, `NO_SUMMARY_STATED`, `KEY_FILE_MAGIC`, `KEY_FILE_VERSION`, `KEY_FILE_LABEL`, `KDF_SCRYPT`, `CIPHER_AES_256_GCM` |

**The first one is pure and the second one is not.** `charter-cli` reaches no runtime
module, reads no file and uses no clock, because everything behind it is `verifier/**`
and that directory may not. It runs unchanged in a browser, a worker or Deno — pass it
bytes and it answers. `charter-cli/producer` is Node-only: it signs with a private key
and draws random bytes, and `node:crypto` is where those come from. `test/api.test.js`
walks both import closures and holds that difference, so the browser-safe claim is a
check rather than a sentence.

**`verify` resolves; it does not throw.** Bytes it cannot read are a *verdict* — four
bytes of nothing come back `BROKEN` with `L0.ZIP.READABLE` failed — so a loop over a
directory branches on `result.verdict` and does not wrap every call in a `try`. The
returned shape is what the documentation fixes, which is why `SPEC.md` §12.1 specifies
those keys and holds the command line's `--json` output to them.

**Anything reachable is importable, and nothing else is.** `charter-cli/verifier/*` is
open on purpose: showing a reader the document and the history inside a file means
parsing a ZIP and a canonical JSON log, and telling you to write your own would be
telling you to disagree with this project about what the bytes are. The two surfaces
above are the stable ones; those modules are available rather than promised. Everything
else — `cli/`, `test/`, `vectors/` — is not exported at all.
...
of what a `VERIFIED` verdict does *not* mean — with a button that saves the
document itself, a write pane that seals one with the key you paste into it, and
an **Append a revision** button that adds an entry to the file that is open, as
`charter edit` does.
...
| Encryption **of the content** | ZIP is not a confidentiality mechanism; a charter's size is visible and the format does not pretend otherwise. The *private key* is a different matter and is worse without protection, which is what `keygen --encrypt` is for (§15.7) — a file at rest, not a claim about an artifact |
...
| `editor/` | a demonstration, not a product: one static page that shows a file's claim, seals one, and appends a revision to one, importing `verifier/**` and nothing else. `editor/serve.mjs` hands it to a browser |
