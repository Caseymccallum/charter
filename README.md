...
npm test                # the 181 tests, through node --test
...
| `docs/spec-declarations.md` | the audit of `SPEC.md`'s list-shaped claims — every rule that names a set (a field list, an enumeration, a ceiling), with the code that declares it again and the test that holds the two together. 36 claims, 20 held, 14 held by nothing; those fourteen are why the next step is a mechanism rather than fourteen more tests |
